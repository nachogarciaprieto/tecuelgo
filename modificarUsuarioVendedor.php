...
<section class="secciones-editables">
    ...
    try{
		...
		$sql= "UPDATE usuarios SET ID_email = :ID_email, password = :password, nombre = :nombre, telefono = :telefono, direccion = :direccion, tipoUsuario = :tipoUsuario, activoUsuario = :activoUsuario WHERE ID_email = :ID_email";

		// Preparo el resultado pasándole la consulta a la conexión. 
		$resultado= $conexion->prepare($sql);

		// Ejecuto el resultado pasándole en un array los parámetros
		$resultado->execute(array(":ID_email"=>$ID_email, ":password"=>$password, ":nombre"=>$nombre, ":telefono"=>$telefono, ":direccion"=>$direccion, ":tipoUsuario"=>$tipoUsuario, ":activoUsuario"=>$activoUsuario));
	...
	echo "<button> <a href='gestionarUsuarios.php'>Volver a Usuarios</a></button>";
